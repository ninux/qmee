...
## Chapter 1 - Experiment 2 GIF creation
...
#{
    ...
    G = bipolar linear potential
    H = boundary reflections
#}
potentialOption = "H";

if (potentialOption == "A")
    # zero potential
    plot_title = "QM FDTD - Moving Electron in Free Space - Zero Potential";
    GIF_FILENAME = "experiment_2_zero_potential.gif";
elseif (potentialOption == "B")
    # constant barrier band in center
    bw = 50;                    # barrier width
    vb = 0.15*eV2J;             # barrier potential
    V(NN/2:NN/2+bw) = vb;
    plot_title = "QM FDTD - Moving Electron in Free Space - Barrier in Center";
    GIF_FILENAME = "experiment_2_barrier_in_center.gif";
elseif (potentialOption == "C")
    ...
    V(1:NN/2) = vb1;
    V(NN/2+1:NN) = vb2;
    plot_title = "QM FDTD - Moving Electron in Free Space - Semiconductor Conduction Band";
    GIF_FILENAME = "experiment_2_semiconductor_conduction_band.gif";
elseif (potentialOption == "D")
    # one-sided finite wall
    vb = 0.2*eV2J;              # potential
    V(NN/2:NN) = vb;
    plot_title = "QM FDTD - Moving Electron in Free Space - One-Sided Finite Well";
    GIF_FILENAME = "experiment_2_one_sided_finite_well.gif";
elseif (potentialOption == "E")
    # linear electric field
    vbm = (0.2/NN)*eV2J;        # max. potential
    V(1:NN) = ([NN:-1:1]')*vbm;
    plot_title = "QM FDTD - Moving Electron in Free Space - Linear Electric Field";
    GIF_FILENAME = "experiment_2_linear_electric_field.gif";
elseif (potentialOption == "F")
    # constant potential
    vbm = (0.2/NN)*eV2J;        # max. potential
    V(1:NN) = ([NN/2:-1:-NN/2+1]')*vbm;
    plot_title = "QM FDTD - Moving Electron in Free Space - Constant Potential";
    GIF_FILENAME = "experiment_2_constant_potential.gif";
elseif (potentialOption == "G")
    # bipolar linear potential
    vb = -0.1*eV2J;             # potential
    V(NN/2+1:NN) = vb;
    plot_title = "QM FDTD - Moving Electron in Free Space - Bipolar Linear Potential";
    GIF_FILENAME = "experiment_2_bipolar_linear_potential.gif";
elseif (potentialOption == "H")
    # boundary reflections
    plot_title = "QM FDTD - Moving Electron in Free Space - Boundary Reflections";
    GIF_FILENAME = "experiment_2_boundary_reflections.gif";
elseif
...
nos     = 100;  # number of timestepts between plots (equidistant)
nop     = 600;  # number of plots to be made
...
while nop > count
    ...
    #legend("real", "imag", "pot");

    axis([1 DX*NN -.2 .2] );

    # insert plot annotations
    text( 5,  .175, sprintf("%7.0f fs",           T*dt*1e15)); # show time in plot
    text( 5, -.175, sprintf("KE = %5.3f eV",      KE));        # show KE in plot
    text(30, -.175, sprintf("PE = %5.3f eV",      PE));        # show PE in plot
    text(30,  .175, sprintf("E_t_o_t = %5.3f eV", KE+PE));     # show Etot in plot
    text(15,  .175, sprintf("<x> = %5.3f nm",     x_avg*1e9)); % show <x> in plot
...
##    endif
    ...
    [imind, cm] = rgb2ind(im);
    if (count == 0)
        imwrite(imind, cm, GIF_FILENAME, "gif", "DelayTime", GIF_DELAY, "Compression", "lzw");
    else
        imwrite(imind, cm, GIF_FILENAME, "gif", "WriteMode", "append", "DelayTime", GIF_DELAY, "Compression", "lzw");
    endif
